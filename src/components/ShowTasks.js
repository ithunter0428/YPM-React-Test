...
import { Container, ListGroup, Dropdown, Form } from "react-bootstrap";
...
const ShowTasks = ({ notes, deleteNote, editNote }) => {
  ...
  useEffect(() => {
    setSortedNotes(notesByOrders[currentOrder].filter(each => each.category.includes(filter) || each.description.includes(filter)));
  }, [filter]);
